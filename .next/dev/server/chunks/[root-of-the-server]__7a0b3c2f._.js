...
"[project]/Desi-Ngl/pages/api/message/reaction.js [api] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function handler(req, res) {
    ...
    if (!messageId || !reaction || !username) {
        return res.status(400).json({
            error: 'Message ID, reaction, and username required'
        });
    }
    ...
    if (!reaction || typeof reaction !== 'string' || reaction.trim().length === 0) {
        return res.status(400).json({
            error: 'Reaction must be a valid emoji'
        });
    }
    ...
    if (reaction.length > 10) {
        return res.status(400).json({
            error: 'Invalid reaction - emoji too long'
        });
    }
    try {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__7a0b3c2f._.js.map